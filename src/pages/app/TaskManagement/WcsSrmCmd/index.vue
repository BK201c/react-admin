...
import mainApi from "@/api/modules/taskManagement/wcsSrmCmdApi";
export default {
mixins: [settingMix],
name  : "wcsSrmCmd",
  data(){
     return {
      ...
      tableTools: [
        ...
        {
          text: "vtableTools.pickReload",
          icon: "el-icon-plus",
          type: "primary",
          authCode: "pickReload",
          click: () => {
            return this.setUsedFlag("pickReload");
          },
        },
        {
          text: "vtableTools.putReload",
          icon: "el-icon-plus",
          type: "primary",
          authCode: "putReload",
          click: () => {
            return this.setUsedFlag("putReload");
          },
        },
        {
        ...
        {
          text: "vtableTools.timeout",
          icon: "el-icon-plus",
          type: "primary",
          authCode: "timeout",
          click: () => {
            return this.setUsedFlag("timeout");
          },
        },
      ...
      disabledListBtns: [
        "vtableTools.delete",
        "vtableTools.reload",
        "vtableTools.pickReload",
        "vtableTools.putReload",
  ...
  methods:{
    ...
    //取货重发
    pickReload(){  
      this.mainApi.cmdPickReload(this.ids).then((response) => {
        if (response.code === 200 || response.code === 0) {
            this.$message.success(response.message);
        } else {
          this.$message.error(response.message);
    ...
    //放货重发
    putReload(){  
      this.mainApi.cmdPutReload(this.ids).then((response) => {
        if (response.code === 200 || response.code === 0) {
            this.$message.success(response.message);
        } else {
          this.$message.error(response.message);
    ...
    //立即超时
    timeout(){  
      this.mainApi.cmdInTimeOut(this.ids).then((response) => {
        if (response.code === 200 || response.code === 0) {
            this.$message.success(response.message);
        } else {
          this.$message.error(response.message);
